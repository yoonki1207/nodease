...
환경변수 (.env):
  LOAD_TEST_DEPLOYMENT_SLUG_2: 테스트할 워크플로우 URL slug
  LOAD_TEST_AUTH_TOKEN_2: 인증 토큰
"""
...
class WorkflowUser(HttpUser):
    ...
    def on_start(self):
        ...
        self.deployment_slug = os.getenv("LOAD_TEST_DEPLOYMENT_SLUG_2")
        self.auth_token = os.getenv("LOAD_TEST_AUTH_TOKEN_2")

        if not self.deployment_slug:
            raise ValueError(
                "LOAD_TEST_DEPLOYMENT_SLUG_2 환경변수가 설정되지 않았습니다"
            )
        if not self.auth_token:
            raise ValueError("LOAD_TEST_AUTH_TOKEN_2 환경변수가 설정되지 않았습니다")
...
def on_test_start(environment, **kwargs):
    """테스트 시작 시 설정 정보 출력"""
    slug = os.getenv("LOAD_TEST_DEPLOYMENT_SLUG_2", "NOT SET")
